...
namespace MuddyTurnip.RulesEngine.Commands
{
    ...
    /// </summary>
    public class MtOatSubstringIndexOperation : OatOperation
    {
        /// <summary>
        /// Create an OatOperation given an analyzer
        /// </summary>
        /// <param name="analyzer">The analyzer context to work with</param>
        public MtOatSubstringIndexOperation(Analyzer analyzer) : base(Operation.Custom, analyzer)
        {
        ...
        public static IEnumerable<Violation> SubstringIndexValidationDelegate(Microsoft.CST.OAT.Rule rule, Clause clause)
        {
            if (clause.Data?.Count == null 
                || clause.Data?.Count == 0)
            {
                yield return new Violation(
                    string.Format(
                        Strings.Get("Err_ClauseNoData"), 
                        rule.Name, 
            ...
            if (clause.DictData != null 
                && clause.DictData?.Count > 0)
            {
                yield return new Violation(
                    string.Format(Strings.Get("Err_ClauseDictDataUnexpected"), 
                        rule.Name, 
                        clause.Label ?? rule.Clauses.IndexOf(clause).ToString(CultureInfo.InvariantCulture), 
                        clause.Operation.ToString()
                    ), 
                    rule, 
                    clause
                );
        ...
        /// <returns></returns>
        public static OperationResult SubstringIndexOperationDelegate(
            Clause clause, 
            object? state1, 
            object? state2, 
            IEnumerable<ClauseCapture>? captures)
        {
            ...
            if (state1 is BlockTextContainer blockTextContainer 
                && clause is OATSubstringIndexClause src)
            {
                if (clause.Data is List<string> stringList 
                    && stringList.Count > 0)
                {
