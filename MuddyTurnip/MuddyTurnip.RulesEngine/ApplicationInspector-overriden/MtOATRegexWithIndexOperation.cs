...
namespace MuddyTurnip.RulesEngine.Commands
{
    ...
    /// </summary>
    public class MtOATRegexWithIndexOperation : OatOperation
    {
        ...
        public MtOATRegexWithIndexOperation(Analyzer analyzer) 
            : base(
                  Operation.Custom, 
                  analyzer)
        {
        ...
        public static IEnumerable<Violation> RegexWithIndexValidationDelegate(
            ...
            Clause clause)
        {
            ...
            else if (clause.Data is List<string> regexList)
            {
                foreach (var regex in regexList)
                {
                    if (!Helpers.IsValidRegex(regex))
                    {
                        yield return new Violation(
                            string.Format(
                                Strings.Get("Err_ClauseInvalidRegex"), 
                                rule.Name, 
                                clause.Label ?? rule.Clauses.IndexOf(clause).ToString(CultureInfo.InvariantCulture), 
                                regex
                            ), 
                ...
                && clause.DictData?.Count > 0)
            {
                yield return new Violation(
                    string.Format(
                        Strings.Get("Err_ClauseDictDataUnexpected"), 
                        rule.Name, 
                        clause.Label ?? rule.Clauses.IndexOf(clause).ToString(CultureInfo.InvariantCulture), 
        ...
        /// <returns></returns>
        public OperationResult RegexWithIndexOperationDelegate(
            Clause clause, 
            object? state1, 
            object? state2, 
        ...
        public Regex? StringToRegex(string built, RegexOptions regexOptions)
        {
            if (!RegexCache.ContainsKey((built, regexOptions)))
            {
                ...
                {
                    Log.Warning(
                        "InvalidArgumentException when creating regex. Regex {0} is invalid and will be skipped.", 
                        built
                    );
